...
export default class implements Command {
    public readonly slashCommand = new SlashCommandBuilder()
        ...
        .addIntegerOption((option) =>
            option.setName("page").setDescription("Page of queue to show [default: 1]").setRequired(false)
        )
        .addIntegerOption((option) =>
            option
                .setName("page-size")
    ...
    public async execute(interaction: ChatInputCommandInteraction) {
        ...
        const embed = buildQueueEmbed(player, interaction.options.getInteger("page") ?? 1, pageSize);
