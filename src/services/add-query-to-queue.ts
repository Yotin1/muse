...
export default class AddQueryToQueue {
    ...
    public async addToQueue({
        ...
        const [targetVoiceChannel] =
            getMemberVoiceChannel(interaction.member as GuildMember) ?? getMostPopularVoiceChannel(interaction.guild!);
        ...
        if (newSongs.length === 1) {
            await interaction.editReply(
                `**${firstSong.title}** added to the${addToFrontOfQueue ? " front of the" : ""} queue${
                    skipCurrentTrack ? "and current track skipped" : ""
                }${extraMsg}`
            );
        } else {
            await interaction.editReply(
                `**${firstSong.title}** and ${newSongs.length - 1} other songs were added to the queue${
                    skipCurrentTrack ? "and current track skipped" : ""
                }${extraMsg}`
            );
        }
    ...
    private async skipNonMusicSegments(song: SongMetadata) {
        ...
        try {
            const segments =
                (await this.cache.wrap(async () => this.sponsorBlock?.getSegments(song.url, ["music_offtopic"]), {
            ...
            if (e.message.includes("504")) {
                ...
                this.sponsorBlockDisabledUntil = new Date(
                    new Date().getTime() + this.sponsorBlockTimeoutDelay * 60_000
                );
            }
